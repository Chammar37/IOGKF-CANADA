...
function event_responsiveness() {
	...
	if($header_display=='header_logo'){
		$event_internal_css = '<!-- Custom CSS -->'."\n";
		$event_internal_css .= '<style type="text/css" media="screen">'."\n";
		$event_internal_css .= 
				'#site-branding #site-title, #site-branding #site-description{
				clip: rect(1px, 1px, 1px, 1px);
				position: absolute;
			}';
		$event_internal_css .= '</style>'."\n";
	}
	if (isset($event_internal_css)) {
		echo $event_internal_css;
	}
}
...
function event_scripts() {
	...
	wp_enqueue_script('event-skip-link-focus-fix', get_template_directory_uri().'/js/skip-link-focus-fix.js', array('jquery'), false, true);
	// Load the html5 shiv.
